		...
		for (int address = 0x2000; address < 0x4000; address++) {
			...
			case 0:
				...
				for (int value = 0; value <= 0xFF; value++) {
					cpu.storeMemory(address, value);
					assertEquals(value, ppu.ppuRegisters.getPpuController(),
							"Writing to PPU Controller should write to the register");
					assertEquals((value & 0b11) << 10, ppu.ppuRegisters.t);
				}

				// Reading (write-only so 0xFF even after modification)
				ppu.ppuRegisters.setPpuController(0x42);
			...
			case 1:
				...
				for (int value = 0; value <= 0xFF; value++) {
					cpu.storeMemory(address, value);
					assertEquals(value, ppu.ppuRegisters.getPpuMask(), "Writing to PPU Mask should write to the register");
				}

				// Reading (write-only so 0xFF even after modification)
				ppu.ppuRegisters.setPpuMask(0x42);
			...
			case 2:
				...
				for (int value = 0; value <= 0xFF; value++) {
					cpu.storeMemory(address, value);
					assertEquals(0, ppu.ppuRegisters.getPpuStatus(), "Writing to PPU Status should not modify the register");
				...
				// Reading
				for (int value = 0; value <= 0xFF; value++) {
					ppu.ppuRegisters.setPpuStatus(value);
					ppu.ppuRegisters.w = 1;
					...
					assertEquals(0, ppu.ppuRegisters.verticalBlankStart,
							"Reading PPU Status should reset the NMI signal (if set)");
					// w always set to 0 after reading
					assertEquals(0, ppu.ppuRegisters.w, "Reading PPU Status should reset the w register");
				}
				break;
			...
			case 4:
				...
				for (int value = 0; value <= 0xFF; value++) {
					...
					assertEquals((value + 1) & 0xFF, ppu.ppuRegisters.ppuOamAddress,
				...
				for (int index = 0; index <= 0xFF; index++) {
					...
					assertEquals(ppu.ppuRegisters.ppuOamData, value,
							"Reading to PPU OAM Data should return the same value than the register");
					assertEquals(ppu.ppuRegisters.ppuOamAddress, index,
			...
			// Write and read
			{
				...
				int value = cpu.fetchMemory(address);
				assertEquals(0x43, ppu.ppuRegisters.ppuOamAddress,
						"Writing to PPU OAM Data should increment PPU OAM Address");
			...
			case 5:
				...
				assertEquals(ppu.ppuRegisters.w, 1, "w must be 1 after one write");
				assertEquals(ppu.ppuRegisters.t, 0x12 >> 3, "t must be written");
				assertEquals(ppu.ppuRegisters.x, 0x12 & 0x3, "t must be written");

				// Second write
				cpu.storeMemory(address, 0x34);
				assertEquals(ppu.ppuRegisters.w, 0, "w must be back to 0 after two writes");
				assertEquals(ppu.ppuRegisters.t, 0b100000011000010, "t must be written");
				assertEquals(ppu.ppuRegisters.ppuScroll, 0b100000011000010, "Write twice to PPU Scroll updates its value");
			...
			case 6:
				...
				cpu.storeMemory(address, 0x12);
				assertEquals(ppu.ppuRegisters.w, 1, "w must be 1 after one write");
				assertEquals(ppu.ppuRegisters.t, 0x12 << 8, "t must be written");

				// Second write
				cpu.storeMemory(address, 0x34);
				assertEquals(ppu.ppuRegisters.w, 0, "w must be back to 0 after two writes");
				assertEquals(ppu.ppuRegisters.ppuAddress, 0x1234, "Write twice to PPU Address updates its value");
				assertEquals(0x6A, ppu.ppuRegisters.ppuData, "Write twice to PPU Address updates PPU Data value");
			...
			case 7:
				...
				assertEquals(0x1235, ppu.ppuRegisters.ppuAddress, "Writing to PPU Data should increment PPU Address by 1");

				// Writing (test increment 32)
				ppu.ppuRegisters.setPpuController(0x04);
				cpu.storeMemory(address, 0x43);
				assertEquals(0x1255, ppu.ppuRegisters.ppuAddress, "Writing to PPU Data should increment PPU Address by 32");

			{
				...
				ppu.ppuRegisters.setPpuController(0x00);
				int value = cpu.fetchMemory(address);
...

			{
				...
				assertEquals(0x1236, ppu.ppuRegisters.ppuAddress, "Writing to PPU Data should increment PPU Address by 2");
			}
				break;
